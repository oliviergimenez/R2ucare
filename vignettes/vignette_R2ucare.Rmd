...
Let's begin by reading in the classical dipper dataset using package `RMark`. The dataset is provided with the package (thanks to Gilbert Marzolin for sharing his data). Note that you do not need to install `MARK` (the executable) at that stage, unless you'd like to actually fit models to data.
...
Let's add spaces between columns:
```{r, message=FALSE, warning=FALSE}
...
Read in the geese dataset using package `RMark`. The dataset is provided with the package (thanks to Jay Hesbeck for sharing his data). Note that you do not need to install `MARK` (the executable) at that stage, unless you'd like to actually fit models to data.
